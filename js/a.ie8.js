/** @license ES6/DOM4 polyfill for IE8 | @version 0.8.2 | MIT License | github.com/termi */
 ...
 * @type {number} @const */
var _browser_msie =  new Function("", "return /*@cc_on true;@*/")() && +((/msie (\d+)/i.exec(navigator.userAgent) || [])[1] || 0) || null;
...
//Reprisent ActiveXObject as Node, Element and HTMLElement so `<element> instanceof Node` is working (!!!But no in IE9 with in "compatible mode")
	__GCC__NODE_CONSTRUCTOR_AS_ACTIVX__ ? global["ActiveXObject"] : __GCC__NODE_CONSTRUCTOR_AS_DOM_ELEMENT__ ? document.createTextNode("") : {}
...
var _ = global["_"]["ielt9shims"]//"_" - container for shims what should be use in a.js
	...
	/** @const */
  , _document_createDocumentFragment = document.createDocumentFragment

	/** @const */
  , _document_createElement = document.createElement

	/** @const */
  , _document_createTextNode = document.createTextNode
...
if(DEBUG) {
	...
	if(!(_Function_call_.call(_document_createElement, document, "div") instanceof ActiveXObject))
...
if(!_tmp_) {
	Function.prototype.apply = function(contexts, args) {
		...
		return _Function_apply_.call(this, contexts, Array["from"](args));
	};
}
	...
	(global["Text"] && global["Text"].prototype ||_Node_prototype).dispatchEvent =
		_Node_prototype.dispatchEvent =
			_document_documentElement.dispatchEvent =
				global.dispatchEvent = document.dispatchEvent =
	function(_event) {
		if( !_event.type ) {
			return true;
		}
...
if(__GCC__UNSTABLE_FUNCTIONS__) {
	_txtTextElement = _Function_call_.call(_document_createTextNode, document, "");
...
safeFragment = html5_document(_Function_call_.call(_document_createDocumentFragment, document));
...
if(_Function_call_.call(_document_createElement, document, "x-x").cloneNode().outerHTML.indexOf("<:x-x>") === 0) {
...
if(!_Element_prototype[_tmp_] && document.querySelectorAll) {
	_Element_prototype[_tmp_] = _document_documentElement[_tmp_] = document[_tmp_] = function(names) {
		...
		return _Function_call_.call(this.querySelectorAll || document.querySelectorAll, this, names.replace(RE_REPLACER_FOR_ELEMENTS_BY_CLASSNAME, "."))
	};
}
