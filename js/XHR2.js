...
//Testing Opera for REAL support xhr.responseType = "document" and xhr.responseType = "blob"
...
if(_XMLHttpRequest_ && IS_XHR_SUPPORT_RESPONSE && global["opera"]) {
	try {
		...
		//Note: We do sync request with xhr.responseType != "" and Opera support it
...
_xhr_onevents.forEach(function(key) {
	...
	this[key] = {
		...
		, "set" : _local__IS_SUPPORN_THIS_ON_EVENT ?
			function(val) {
				return this.nativeXHR[key] = val ? val.bind(this) : val;
			}
			:
			function(val) {
				this["_"]["__on__" + key] = val ? val.bind(this) : val;
	...
	if(cors && cors.state > 1) {
		if(!__GCC__IELT10_SUPPORT__ || !IS_XDOMAINREQUEST_SUPPORT) {
			...
			if(xhr.withCredentials) {
				xhr = new XDomainRequest;//TODO:: this just woudn't work. Need more comples solution
			}
			else {
	...
	XMLHttpRequest2_createNewCORSXhr = function(xhr, corsDescription) {
		...
		if(__GCC__IELT10_SUPPORT__ && IS_XDOMAINREQUEST_SUPPORT) {
			if(!xhr.withCredentials) {
				...
				corsXhr.xhrType = 3;//cors via XDomainRequest
			}	
		}
