...
if(INCLUDE_DOMSTRINGCOLLECTION) {
	 ...
	 * @constructor
	 */
	DOMStringCollection_ = function(string, getter, setter, object_this) {
		/**
	...
	get__DOMStringCollection_property = function(thisObj, attributeName) {
		...
		if(!_currentValue) {
			_currentValue = _[attributeName] = new DOMStringCollection_(function() {
				thisObj.getAttribute(attributeName);
			}, function() {
		...
		// --- === HTMLPropertiesCollection CLASS [BEGIN] === ---
			...
			HTMLPropertiesCollection.prototype._clear = function() {
				...
				for(var i in thisObj) {
					if(thisObj[i] instanceof PropertyNodeList || !isNaN(i)) {
						thisObj[i] = null;
						delete thisObj[i];
					}
				}
			 ...
			 */
			HTMLPropertiesCollection.prototype["_push"] = function(node) {
				var thisObj = this,
